...
| Метрика | PostgreSQL | Tarantool | Δ |
|---|---|---|---|
| `List` med | 3.45 ms | 2.88 ms | −16.4% |
| `List` p95 | 7.56 ms | 7.00 ms | −7.4% |
| `List` p99 | 11.12 ms | 11.17 ms | +0.5% |
| `Send` med | 12.24 ms | 2.99 ms | −75.5% |
| `Send` p95 | 70.58 ms | 6.80 ms | −90.4% |
| `Send` p99 | 79.86 ms | 11.40 ms | −85.7% |
| Итераций/с | 683.4 | 736.6 | +7.8% |

...
| `Dialogs/MarkRead` | Сдвинуть отметку прочтения диалога, вернуть новый watermark |
| `Dialogs/GetUnreadCounts` | Вернуть непрочитанное по собеседникам — дешёвая проверка расхождений |
| `Dialogs/GetUnreadState` | Вернуть идентификаторы непрочитанных сообщений — по ним сервис счётчиков пересобирает своё состояние |
